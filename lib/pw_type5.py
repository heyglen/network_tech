...
def decode(type_5_password):
    _, _, salt, password_hash = type_5_password.split('$')
    
    for bad_password in _BAD_PASSWORDS:
        bad_password_hash = _decode(bad_password.encode('ascii'), salt)
        # print(f'{bad_password} {bad_password_hash} == {password_hash}')
        if bad_password_hash == password_hash:
            return bad_password
    return 'Brute force decode failed'
